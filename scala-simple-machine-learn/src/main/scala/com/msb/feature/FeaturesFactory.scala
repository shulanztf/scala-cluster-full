...
//    读取hbase信息,扫描信息
    ...
    val distinctWords = hbaseRdd.flatMap(data => {
      ...
      .zipWithIndex()//生成全局序号，连续
      .collectAsMap()

    val distinctWordsBroad = session.sparkContext.broadcast(distinctWords) // 广播变量
    ...
    //省市处理
    val provinceWithCity:RDD[(String,(String,String))] = hbaseRdd.map(data => {
      val result = data._2
      var userID = ""
      var province = ""
      var city = ""
    ...
    val trainDF = assem.setInputCols(Array("program_features", "province_Vector", "city_Vector", "userLabel_Vector"))
